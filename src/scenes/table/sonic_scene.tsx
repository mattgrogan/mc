...
import datac from "../../../../dicedata/output/sonic_the_hedgehog-100k/sonic_the_hedgehog-100k-overall_stats.json";
...
export default makeScene2D(function* (view) {
  ...
  view.add(
    <Table
      ...
      headerTxtProps={{ ...PoppinsWhite, fontSize: 35, fontWeight: 700 }}
      CellProps={{
      ...
      CellTxtProps={{ ...PoppinsWhite, fontSize: 35 }}
      height={850}
      y={0}
      numberFormat={[
        {
  ...
  // Highlight First Row
  yield* tabled().highlightRow(1, { fill: tw_colors.yellow[700] });
  yield* waitFor(3);
  yield* tabled().removeHighlighters();
  //   yield* waitFor(1);
  // Highlight Second Row
  yield* tabled().highlightRow(2, { fill: tw_colors.yellow[700] });
  yield* waitFor(3);
  yield* tabled().removeHighlighters();

  yield* waitFor(1);
  yield* tabled().scrollToRow(36, 5);
