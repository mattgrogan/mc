...
import { data, name } from "./DD_00_Params";
...
const DATA = data.OVERALL_STATS.ColdTable;
...
const TOTAL_BET = DATA.TOTAL_BET;
...
export default makeScene2D(function* (view) {
  ...
  rowValues[0].text(commaFormmatter(DATA.DROP));
  rowValues[1].text(commaFormmatter(TOTAL_BET));
  rowValues[2].text(plusCommaFormmatter(DATA.TOTAL_WON));
  rowValues[3].text(commaFormmatter(DATA.TOTAL_LOST));
  rowValues[4].text(commaFormmatter(DATA.HOUSE_TAKE));
  rowValues[5].text(commaFormmatter(DATA.HOUSE_EDGE * 100, 3) + "%");
  // ADD THE PLOT AREA
  ...
  yield plotTitle.container.opacity(0, 0.6);
  yield camera().centerOn([700, 165], 4, easeInOutCubic);
  yield camera().zoom(1.2, 4, easeInOutCubic);
  yield* waitFor(1);

  // ************************
  // THIS STRATEGY
  // ************************
  const HOUSE_EDGE_PERCENT = DATA.HOUSE_EDGE * 100;
  ...
  yield plotTitle.container.opacity(1, 1.5);
