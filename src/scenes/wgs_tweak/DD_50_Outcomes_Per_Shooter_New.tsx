...
import byhand from "../../../../dicedata/output/wgs_tweak-100k-newreport/json/outcomes_by_hand.v1.json";
import bySession from "../../../../dicedata/output/wgs_tweak-100k-newreport/json/outcomes_by_session.v1.json";
import { tw_colors } from "../../tw_colors";

const byHandData = byhand.PLYR;
const bySessionData = bySession.PLYR;
...
export default makeScene2D(function* (view) {
  ...
  const winLossBar2 = createRef<Layout>();
  const totalWon2 = createSignal(0);
  const totalLoss2 = createSignal(0);

  view.add(
    <WinLossBar
      ref={winLossBar}
      title="Outcomes by Hand"
  ...
  yield* totalWon(byHandData.N_UP, 2);
  yield* totalLoss(byHandData.N_DOWN, 2);

  yield* waitFor(1);
  yield* FadeIn(winLossBar2, 1, easeInOutCubic, [0, 100]);
  // Animate the numbers growing
  yield* totalWon2(bySessionData.N_UP, 2);
  yield* totalLoss2(bySessionData.N_DOWN, 2);
