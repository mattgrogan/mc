...
// hist_session_outlay.v1.json
import dataImport from "../../../../dicedata/output/y2025/m07/fivealive-100k/json/hist_session_outlay.v1.json";
// quantiles_session_outlay.v1.json
import quantilesImport from "../../../../dicedata/output/y2025/m07/fivealive-100k/json/quantiles_session_outlay.v1.json";

const DATA = dataImport[PLAYER_NAME];
const QUANTILES = quantilesImport[PLAYER_NAME];

const title = createSignal("Total Outlay per Session");
const subtitle = createSignal(
  `Distribution of total out-of-pocket across ${commaFormmatter(
    QUANTILES.N
  )} sessions.`
);
...
export default makeScene2D(function* (view) {
  ...
  // --------------- zeroLine ----------------
  const zeroLine = plot().vLine([0, Y_AXIS_MAX * 0.8], {
    lineWidth: 5,
    stroke: Grays.GRAY2,
    lineDash: [20, 5],
    opacity: 0.5,
