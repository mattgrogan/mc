...
export default makeScene2D(function* (view) {
  ...
  // Process data from the correct structure
  // bbData.BOTS contains the bot info
  // bbData.THROWS contains all the throws, each with BOT_THROWS for all bots at that SESSION_ROLL

  const botList = bbData.BOTS;
  const throws = bbData.THROWS;

  // Create a lookup for bot data by session for each throw
  const botDataByThrow: Record<number, Record<number, any>> = {};

  for (const throwData of throws) {
    botDataByThrow[throwData.SESSION_ROLL] = {};
    for (const botThrow of throwData.BOT_THROWS) {
      botDataByThrow[throwData.SESSION_ROLL][botThrow.SESSION] = botThrow;
    }
  }

  // Calculate axis limits from all bot throws
  const allBankrolls: number[] = [];
  const allRolls: number[] = [];

  for (const throwData of throws) {
    allRolls.push(throwData.SESSION_ROLL);
    for (const botThrow of throwData.BOT_THROWS) {
      allBankrolls.push(botThrow.NET_BR_END);
    }
  }
  ...
  for (let i = 0; i < botList.length; i++) {
    ...
    // Create line coordinates by collecting data for this bot across all throws
    const coords: Vector2[] = [];
    for (const throwData of throws) {
      const botThrow = throwData.BOT_THROWS.find(
        (bt) => bt.SESSION === bot.SESSION
      );
      if (botThrow) {
        coords.push(
          new Vector2(
            botThrow.SESSION_ROLL + bot.SESSION / 20, // Small offset to prevent overlap
            botThrow.NET_BR_END + bot.SESSION * 10 // Small Y offset to prevent overlap
          )
        );
      }
    }
  ...
  // Group throws by update intervals to avoid multiple updates within the same interval
  const throwsByInterval: Record<number, (typeof throws)[0]> = {};

  for (const throwData of throws) {
    const intervalKey = Math.floor(
      (throwData.SESSION_ROLL * ANIMATION_CONFIG.SECS_PER_ROLL) /
        ANIMATION_CONFIG.RANK_UPDATE_INTERVAL
    );
    // Keep the latest throw in each interval (this will be the final state for that interval)
    throwsByInterval[intervalKey] = throwData;
  }

  // Update leaderboard - but only once per interval using the final throw data
  for (const throwData of throws) {
    const delayTime = throwData.SESSION_ROLL * ANIMATION_CONFIG.SECS_PER_ROLL;
    const intervalKey = Math.floor(
      delayTime / ANIMATION_CONFIG.RANK_UPDATE_INTERVAL
    );

    // Only update ranks if this is the final throw for this interval
    const isLastThrowInInterval = throwsByInterval[intervalKey] === throwData;

    // Update each bot's signals based on this throw's data
    for (let i = 0; i < botList.length; i++) {
      const bot = botList[i];
      const botThrow = throwData.BOT_THROWS.find(
        (bt) => bt.SESSION === bot.SESSION
      ...
      if (botThrow) {
        ...
        if (isLastThrowInInterval) {
          animationGenerators.push(
            delay(
              delayTime,
              botSignals[i].ordinalRankSignal(
                botThrow.ORDINAL_RANK,
        ...
        // Mark as finished if this is the bot's final roll
        if (botThrow.SESSION_ROLL === botThrow.MAX_SESSION_ROLL) {
          animationGenerators.push(
  ...
  // TODO: Add X-axis scaling code here to match the rank update intervals
  // This is where you should add the code for updating x-axis ticks/scaling
  ...
  // Get final rankings by finding the last throw data
  const finalThrow = throws[throws.length - 1];
  const finalRankings = finalThrow.BOT_THROWS.sort(
    (a, b) => a.ORDINAL_RANK - b.ORDINAL_RANK
  ).slice(0, 3); // Top 3

  // Highlight each winner in sequence (1st, 2nd, 3rd place)
  for (let rank = 0; rank < 3; rank++) {
    const winnerBotThrow = finalRankings[rank];

    // Find the bot index in our arrays
    const botIndex = botList.findIndex(
      (bot) => bot.SESSION === winnerBotThrow.SESSION
    );
