...
export default makeScene2D(function* (view) {
  ...
  const leaderboards: Record<number, Rect> = {};
  ...
  for (const session in bankrollCoords) {
    lines[session] = plot().steppedLine([0, 0], bankrollCoords[session], {
      lineWidth: 5,
      opacity: 0.8,
  ...
  for (let i = 0; i < 250; i++) {
    ...
    for (const bot of rollData) {
      ...
      yield pctComplete[bot.SESSION](bot.PCT_COMPLETE, DELAY, linear);
    }
